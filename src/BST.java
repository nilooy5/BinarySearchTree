...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private Node ceiling(Node x, Key key) {
        StdOut.println("Key: " + x.key + " & value: " + x.val);
        if (x == null) return null;
        int cmp = key.compareTo(x.key);
        if (cmp == 0) return x;
        if (cmp < 0) {
    ...
    /**
     * @param x root Node
     * @param rank of the selecting node
     * @return key in BST rooted at x of given rank.
     */
    private Key select(Node x, int rank) {
        // StdOut.println("key: " + x.key + " & rank: " + rank);
        if (x == null) return null;
    ...
    private Key selectRandomKey(int height, BST st) {
        Random rand = new Random();
        int rand_int = rand.nextInt(height);
         return (Key) st.select(rand_int);
    }
    ...
    public static void main(String[] args) {
        ...
        StdOut.println("Assignment 7, ex 4");
        ...
        // StdOut.println(st.ceiling("Q"));
        // StdOut.println(st.rank("J"));
        // StdOut.println(st.size("D", "T"));
        // StdOut.println(st.keys("D", "T"));
        StdOut.println(st.selectRandomKey(st.height(), st));
    }
}
